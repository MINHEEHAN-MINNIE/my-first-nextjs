...
| `(END)` 에서 안 나가짐 | less (페이저) | `q`로 나가야 함 |
...
**원인** — `git log`처럼 출력이 화면보다 길면 git이 **페이저**(기본값 `less`)에 넣어서 보여준다. `(END)`는 문서 끝에 도달했다는 표시이고, 그 상태에서 친 글자는 less가 **명령으로 해석**한다. `Ctrl + C`는 less가 무시한다.

**해결** — `q` (quit)

**배운 것** — 원래 `more`라는 프로그램이 있었고, 개선판 이름을 말장난으로 `less`라고 붙였다. 기능은 more보다 많다.

조작: `스페이스` 다음 화면 / `↑`,`↓` 한 줄씩 / `G` 맨 끝 / `g` 맨 앞 / `/단어` 검색 / `q` 나가기

**예방** — 일상적으로는 짧게 본다. `git log --oneline -10` 정도면 화면에 들어가서 페이저가 안 뜬다.
이번만 끄려면 `git --no-pager log`, 계속 끄려면 `git config --global core.pager cat` (긴 로그가 쏟아지니 권하진 않음)

---

# 터미널 탈출 표

Day 1에 네 종류를 다 만났다. 사실상 터미널 생존 지식의 전부다.

| 상황 | 화면 표시 | 탈출 |
|---|---|---|
| 페이저 (`git log`, `--help`) | `:` 또는 `(END)` | **`q`** |
| Vim 편집기 | `~` 줄들 | **`:q!`** (저장하려면 `:wq`) |
| 프로그램 실행 중 (개발 서버) | 프롬프트 없음 | **`Ctrl + C`** |
| 셸이 입력 대기 | `dquote>`, `>` | **`Ctrl + C`** |

**한 줄로 줄이면** — 글자를 보여주고 있으면 `q`, 뭔가 돌아가고 있으면 `Ctrl + C`.
